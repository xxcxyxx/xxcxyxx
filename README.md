<p align="center">
  <h2 align="center">📚 Tech Stack</h2>
</p>

<p align="center">
  <img src="https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/SQL-003B57?style=flat&logo=sqlite&logoColor=white" />
  <img src="https://img.shields.io/badge/Pandas-150458?style=flat&logo=pandas&logoColor=white" />
  <img src="https://img.shields.io/badge/NumPy-013243?style=flat&logo=numpy&logoColor=white" />
  <img src="https://img.shields.io/badge/ScikitLearn-F7931E?style=flat&logo=scikit-learn&logoColor=white" />
  <img src="https://img.shields.io/badge/CatBoost-FFCC00?style=flat&logoColor=black" />
</p>

<p align="center">
  <img src="https://img.shields.io/badge/Jupyter-F37626?style=flat&logo=jupyter&logoColor=white" />
  <img src="https://img.shields.io/badge/Google_Colab-F9AB00?style=flat&logo=googlecolab&logoColor=white" />
  <img src="https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white" />
</p>

<br>

<p align="center">
  <h2 align="center">⛵ About Me</h2>
</p>

<p align="center">
  데이터를 수집하고 정리하여 분석 가능한 형태로 만드는 과정에 관심이 있습니다.<br>
  이벤트 로그, 시계열 데이터 등 다양한 형태의 데이터를 다루며<br>
  패턴을 해석하고 인사이트로 연결하는 경험을 쌓아왔습니다.
</p>

<p align="center">
  특히 데이터 구조를 설계하고 전처리부터 모델링, 결과 해석까지<br>
  전 과정을 직접 수행하는 것을 중요하게 생각합니다.
</p>

<br>

<p align="center">
  <h2 align="center">📂 Main Projects</h2>
</p>

<p align="center">
  ⚽ <b>K League Pass Prediction</b><br>
  이벤트 로그 데이터를 재구성하여 패스 도착 좌표를 예측하고,<br>
  공간 패턴과 오차 특성을 분석한 프로젝트
</p>

<p align="center">
  <a href="https://github.com/xxcxyxx/2025KLeaguePassPrediction">
    <img src="https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white" />
  </a>
</p>

<br>

<p align="center">
  🧬 <b>Lung Cancer Gene Prediction</b><br>
  유전자 데이터를 기반으로 폐암 아종을 분류하는 딥러닝 모델 연구 프로젝트
</p>

<p align="center">
  <a href="https://github.com/xxcxyxx/Pre-training-with-Augmentation-Data-in-Deep-Learning-Model">
    <img src="https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white" />
  </a>
<a href="https://github.com/xxcxyxx/Pre-training-with-Augmentation-Data-in-Deep-Learning-Model/blob/main/paper/lung_cancer_paper.pdf">
  <img src="https://img.shields.io/badge/Paper-F4C2C2?style=flat&logo=readthedocs&logoColor=white" />
  </a>
</p>

<br>

<p align="center">
  <h2 align="center">📧 Contact me</h2>
</p>

<p align="center">
  <a href="mailto:co505123060@gmail.com">
    <img src="https://img.shields.io/badge/Gmail-D14836?style=flat&logo=gmail&logoColor=white" />
  </a>
  <a href="mailto:co5051230@naver.com">
    <img src="https://img.shields.io/badge/Naver-03C75A?style=flat&logo=naver&logoColor=white" />
  </a>
</p>
